...
class ShortcutManager: ObservableObject, Manager {
    ...
    @Published var ecounterProjectorOpen: Bool = false {
        didSet {
            if !ecounterProjectorOpen {
                NSApplication.shared.windows.first(where: { $0.title == "E Counter Projector" })?.close()
            }
        }
    }
    ...
    @Published var pieProjectorOpen: Bool = false {
        didSet {
            if !pieProjectorOpen {
                NSApplication.shared.windows.first(where: { $0.title == "Pie Projector" })?.close()
            }
        }
    }
    ...
    func resizeThin() {
        guard let pid = activeInstancePID() else { return }
        let (w, h, x, y) = Settings[\.self].thinDimensions(for: TrackingManager.shared.trackedInstances.first { $0.pid == pid })
        if resize(pid: pid, x: x, y: y, width: w, height: h) == true {
            if let instance = TrackingManager.shared.trackedInstances.first(where: { $0.pid == pid }
                ...
                ScreenRecorder.shared.ProjectedInstance = instance
                Task(priority: .userInitiated) {
                    await ScreenRecorder.shared.startECounterProjectorCapture(for: instance, mode: .thin)
                    if Settings[\.utility].ecounterProjectorEnabled{
                        ecounterProjectorOpen = true
                    }
                }
                Task(priority: .userInitiated) {
                    await ScreenRecorder.shared.startPieProjectorCapture(for: instance, mode: .thin)
                    if Settings[\.utility].pieProjectorEnabled{
            ...
            if resize(pid: pid, x: x, y: y, width: w, height: h) == true {
                if let instance = TrackingManager.shared.trackedInstances.first(where: { $0.pid == pid }
                    ...
                    Task(priority: .userInitiated) {
                        ...
                        await ScreenRecorder.shared.startECounterProjectorCapture(for: instance, mode: .tall)
                        if Settings[\.utility].ecounterProjectorShouldOpenWithTallMode {
                            ecounterProjectorOpen = true
                        }
                        await ScreenRecorder.shared.startPieProjectorCapture(for: instance, mode: .tall)
                        if Settings[\.utility].pieProjectorShouldOpenWithTallMode {
                            pieProjectorOpen = true
                        }
                    }
                }
        ...
        @discardableResult func resize(
               ...
               let currentPosition = WindowController.getWindowPosition(pid: pid)
            {
                ...
                let (tallw, tallh, _, _) = Settings[\.self].tallDimensions(for: TrackingManager.shared.trackedInstances.first {$0.pid == pid})
                if currentSize == CGSize(width: tallw, height: tallh) {
                    Task(priority: .userInitiated) {
                        await ScreenRecorder.shared.stopEyeProjectorCapture()
                        ScreenRecorder.shared.ProjectedInstance = nil
                    }
                    if Settings[\.utility].eyeProjectorShouldOpenWithTallMode {
                        eyeProjectorOpen = false
                    }
                    Task(priority: .userInitiated) {
                        await ScreenRecorder.shared.stopECounterProjectorCapture()
                        ScreenRecorder.shared.ProjectedInstance = nil
                    }
                    if Settings[\.utility].ecounterProjectorShouldOpenWithTallMode {
                        ecounterProjectorOpen = false
                    }
                    Task(priority: .userInitiated) {
                        await ScreenRecorder.shared.stopPieProjectorCapture()
                        ScreenRecorder.shared.ProjectedInstance = nil
                    }
                    if Settings[\.utility].pieProjectorShouldOpenWithTallMode {
                ...
                // detect exiting thin mode
                let (thinw, thinh, _, _) = Settings[\.self].thinDimensions(for: TrackingManager.shared.trackedInstances.first {$0.pid == pid})
                if currentSize == CGSize(width: thinw, height: thinh) {
                    Task(priority: .userInitiated) {
                        await ScreenRecorder.shared.stopECounterProjectorCapture()
                        ScreenRecorder.shared.ProjectedInstance = nil
                        ecounterProjectorOpen = false
