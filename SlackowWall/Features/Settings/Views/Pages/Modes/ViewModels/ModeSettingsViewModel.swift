...
class ModeSettingsViewModel: ObservableObject {
    ...
    var multipleOutOfBounds: Bool {
        let p = Settings[\.self]
        let dimensions = [
            p.baseDimensions, p.tallDimensions(for: TrackingManager.shared.trackedInstances.first), p.thinDimensions(for: TrackingManager.shared.trackedInstances.first), p.wideDimensions, p.resetDimensions
        ]
